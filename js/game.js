...
const phishingEmails = [
  ...
  document.addEventListener('keydown', (e) => {
    if (e.key === 'Escape') {
      [phishingGameModal, passwordGameModal, networkGameModal].forEach(modal => {
        if (!modal.classList.contains('hidden')) {
          hideModal(modal);
          if (modal === phishingGameModal) resetPhishingGame();
          else if (modal === passwordGameModal) resetPasswordGame();
          else if (modal === networkGameModal) resetNetworkGame();
        }
      });
    }
  });
